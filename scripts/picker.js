...
async function startPicker(args) {
  const folders = await Promise.all(
    (
      await fs.readdir(new URL("../slides", import.meta.url), {
        withFileTypes: true,
      })
    )
      ...
      .map(async (folder) => {
        const md = await fs.readFile(
          new URL(`../slides/${folder}/README.md`, import.meta.url),
          "utf-8",
        );
  ...
  if (result.folder) {
    ...
    //   execa("cursor", [
    //     fileURLToPath(
    //       new URL(`../slides/${result.folder}/slides.md`, import.meta.url),
    //     ),
    //   ]);
